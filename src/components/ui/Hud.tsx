...
export default function Hud() {
  const { hearts, level, missionsCompleted, characterId } = useGameStore()
  const [menuOpen, setMenuOpen] = useState(false)

  const levelData = levelsData.find((l) => l.level === level) ?? levelsData[0]
  const progress  = Math.min(missionsCompleted.length / TOTAL_MISSIONS, 1)
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-0.5">
              <span className="text-[11px] font-bold text-[#7BA577] truncate leading-none">
                {levelData.name}
              </span>
              <span className="text-[10px] text-[#3D2E1F]/40 font-semibold ml-1 flex-shrink-0 leading-none">
                {missionsCompleted.length}/{TOTAL_MISSIONS}
              </span>
            </div>
